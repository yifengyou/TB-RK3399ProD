...
git clone --progress https://github.com/ophub/linux-6.1.y.git linux-6.1.y.git
...
cd linux-6.1.y.git
# apply patch
if ls "${BUILDER_DIR}/kernel-6.1/"*.patch >/dev/null 2>&1; then
  git config --global user.name yifengyou
  git config --global user.email 842056007@qq.com
  git am ${BUILDER_DIR}/kernel-6.1/*.patch
fi
# config kernel
if [ -f ${BUILDER_DIR}/kernel-6.1/config-6.1 ];then
  cp -a ${BUILDER_DIR}/kernel-6.1/config-6.1 .config
fi
