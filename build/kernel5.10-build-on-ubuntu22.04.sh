...
git clone --progress https://github.com/ophub/linux-5.10.y.git linux-5.10.y.git
...
cd linux-5.10.y.git
# apply patch
if ls "${BUILDER_DIR}/kernel-5.10/"*.patch >/dev/null 2>&1; then
  git config --global user.name yifengyou
  git config --global user.email 842056007@qq.com
  git am ${BUILDER_DIR}/kernel-5.10/*.patch
fi
# config kernel
if [ -f ${BUILDER_DIR}/kernel-5.10/config-5.10 ];then
